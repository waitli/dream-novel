...
设计${params.numberOfChapters}章的**详细节奏分布**（根据小说类型调整风格）：
...
## 每章详细设计（严格按照以下格式，每章简述必须 200-300 字）

第 n 章 - [章节标题，10-20 字，有吸引力]
【本章定位】[角色视角/关键事件/世界观揭示/情感转折，20 字]
【核心作用】[推进主线/角色成长/关系变化/伏笔埋设，30 字]
【情感基调】[紧张/温馨/悬疑/悲伤等，符合小说类型，15 字]
【出场角色】[列出本章所有出场角色及其状态变化，50 字]
【场景设计】[主要场景 1-3 个，包含地点、时间、氛围，50 字]
【情节要点】
  1. 开场：[如何引入本章，20 字]
  2. 发展：[核心事件展开，40 字]
  3. 转折：[意外或冲突点，30 字]
...
【悬念密度】[★☆☆☆☆～★★★★★，说明悬念类型]
【情节张力】[★☆☆☆☆～★★★★★，说明张力来源]
【关键台词】[1-2 句本章最令人印象深刻的台词]
【本章简述】[200-300 字详细概括，包含具体事件和角色变化]

## 输出要求
1. **每章简述必须达到 200-300 字**，包含具体情节发展
2. 伏笔必须标注预计回收章节（如：伏笔 A→第 50 章回收）
3. 角色变化必须具体（如：从怀疑→信任，从陌生→爱慕）
4. 场景设计要包含感官细节（视觉/听觉/嗅觉）
5. **严格遵循小说架构中的角色设定和世界观**
6. 在生成${params.numberOfChapters}章前不要出现结局章节

**现在开始生成${params.numberOfChapters}章的详细大纲。**
...
需要生成总共${params.numberOfChapters}章的详细节奏分布，

当前已有章节目录（若为空则说明是初始生成）：
${params.chapterList || '(无)'}

现在请设计第${params.startChapter}章到第${params.endChapter}章的详细节奏分布：

## 每章详细设计（严格按照以下格式，每章简述 200-300 字）
